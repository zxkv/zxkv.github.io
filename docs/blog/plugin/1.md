...
因为是 **Web 端**、**H5 端**和**小程序端**都需要用到这组数据，**小程序**和**H5 端**采用了 **VantUI**进行开发，同时有看到 **VantUI**组件库里有**省市区数据连联动的组件**，相对这两端来说，有现成的组件使用，开发起来还是比较快的，但是 **Web 端** 需要使用数据，最终为了保持三端数据一致，就采用了 移动端的 **VantUI** 地区联动组件的数据。
...
通过本次对地区数据的封装，可以依据场景和业务需求引用自己所需的数据格式；初次封装工具包，难免有些地方需要改进。本次主要分享包的使用，希望对大家有所帮助。有好的建议可以留言，初版难免会有考虑不周的地方，可留言提出宝贵意见或建议。🙏
